...
async def create_project(
    ...
    return mask_scores(await crud_project.create_project(db, project_data, current_user.id), current_user)
...
async def read_projects_by_faculty(
    ...
    return mask_scores(await crud_project.get_projects_by_faculty(db, faculty_id), current_user)
...
async def update_project(
    ...
    project_update: ProjectPartAUpdateFaculty | ProjectPartAUpdateHOD | ProjectPartAUpdateDirector | ProjectPartAUpdateDean | ProjectPartAUpdateVC,
):
    ...
    if "vc" in current_user.roles:
        res = await crud_project.update_project_vc(db, id, project_update)
    elif "dean" in current_user.roles:
        res = await crud_project.update_project_dean(db, id, project_update)
    elif "director" in current_user.roles:
        res = await crud_project.update_project_director(db, id, project_update)
    elif "admin" in current_user.roles or "hod" in current_user.roles:
        res = await crud_project.update_project_hod(db, id, project_update)
    elif "faculty" in current_user.roles and db_entry.faculty_id == current_user.id:
        res = await crud_project.update_project_faculty(db, id, project_update)
    else:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized")
