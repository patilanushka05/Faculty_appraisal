...
async def create_teaching_process(
    ...
    res = await crud_teaching_process.create_teaching_process(db, teaching_data, current_user.id)
...
async def read_teaching_process_by_faculty(
    ...
    res = await crud_teaching_process.get_teaching_process_by_faculty(db, faculty_id)
    return mask_scores(res, current_user)
...
async def update_teaching_process(
    ...
    teaching_update: Union[TeachingProcessUpdateFaculty, TeachingProcessUpdateHOD, TeachingProcessUpdateDirector, TeachingProcessUpdateDean, TeachingProcessUpdateVC],
):
    ...
    if "admin" in current_user.roles:
        # Admin can update everything, but we branch to maintain score audit
        res = await crud_teaching_process.update_teaching_process_hod(db, id, teaching_update)
    elif "vc" in current_user.roles:
        res = await crud_teaching_process.update_teaching_process_vc(db, id, teaching_update)
    elif "dean" in current_user.roles:
        res = await crud_teaching_process.update_teaching_process_dean(db, id, teaching_update)
    elif "director" in current_user.roles:
        res = await crud_teaching_process.update_teaching_process_director(db, id, teaching_update)
    elif "hod" in current_user.roles:
        res = await crud_teaching_process.update_teaching_process_hod(db, id, teaching_update)
    elif "faculty" in current_user.roles and db_teaching.faculty_id == current_user.id:
        res = await crud_teaching_process.update_teaching_process_faculty(db, id, teaching_update)
    else:
...
async def read_teaching_process_summary(
    ...
    return {
        ...
        "scaledMarksOutOf25": min(total_score * 0.25, 25)
    }
